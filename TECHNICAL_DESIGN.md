...
| LLM | Cloudflare Workers AI | 要約・スコア・タグ生成 |
...
### 9.1 Workers AI バインディング

Workers AI は wrangler.toml の `[ai]` バインディングで利用する。外部APIキー不要。
...
Ingest時のLLM呼び出しは**1リクエスト**に統合する（レイテンシ最適化）。
...
| 用途 | モデル | 理由 |
|------|--------|------|
| 要約・タグ・スコア | `@cf/meta/llama-3.1-70b-instruct` | JSON出力の安定性、多言語対応 |
| embedding | `@cf/baai/bge-base-en-v1.5` | Vectorize標準、768次元 |

- 70Bモデルの品質が不十分な場合は `@cf/meta/llama-3.3-70b-instruct-fp8-fast` を検討
- Workers AI は Cloudflare ネットワーク内で完結するため、外部API呼び出しより低レイテンシ

### 9.4 コスト見積もり

Workers AI の pricing（Neurons単位）:

| 項目 | 概算 |
|------|------|
| llama-3.1-70b: ~3500 input tokens | ~130 Neurons/記事 |
| llama-3.1-70b: ~200 output tokens | ~650 Neurons/記事 |
| bge-base embedding | ~20 Neurons/記事 |
| 月100記事の場合 | ~80,000 Neurons/月 |
| Workers Paid ($5/mo) 付属分 | 10,000 Neurons/日 = ~300,000/月 |

月100記事程度なら Workers Paid プランの付属枠内に収まる。
...
│   ├── llm/
│   │   └── workers-ai.ts     -- Workers AI クライアント
│   └── utils/
│       ├── url.ts            -- URL正規化・ハッシュ
