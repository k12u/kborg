...
function renderCard(item: Item): HTMLElement {
  ...
  card.innerHTML = `
    ...
    <div class="card-meta">
      <span class="badge badge-blue">org: ${Math.round(item.org_score * 100)}%</span>
...
export function renderOrg(container: HTMLElement): void {
  ...
  async function loadMore(): Promise<void> {
    ...
    try {
      const result = await fetchItems('org', cursor);
